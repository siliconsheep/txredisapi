...
def list_or_args(command, keys, args):
    ...
    try:
        iter(keys)
        if isinstance(keys, six.string_types) or isinstance(keys, six.binary_type):
            keys = [keys]
            if not oldapi:
                return keys
            oldapi = True
    ...
    if oldapi:
        warnings.warn(DeprecationWarning("Passing *args to redis.%s is deprecated. " "Pass an iterable to ``keys`` instead" % command))
        keys.extend(args)
    return keys
...
class BaseRedisProtocol(LineReceiver):
    ...
    def __init__(self, charset="utf-8", errors="strict", replyTimeout=None, password=None, dbid=None, convertNumbers=True):
    ...
    def connectionMade(self):
        ...
        if self.dbid is not None:
            try:
                ...
                msg = "Redis error: could not set dbid=%s: %s" % (self.dbid, str(e))
                self.factory.connectionError(msg)
                if self.factory.isLazy:
                    log.msg(msg)
                defer.returnValue(None)
            ...
            if data:
                ...
                try:
                    ...
                    if num_data[0] in _NUM_FIRST_CHARS:  # Most likely a number
                        try:
                            el = int(num_data) if num_data.find(".") == -1 else float(num_data)
                        except ValueError:
                            pass
    ...
    def _encode_value(self, arg):
        if isinstance(arg, six.binary_type):
            ...
            try:
                ...
                raise InvalidData("Error encoding unicode value '%s': %s" % (repr(arg), e))
    ...
    def flush(self, all_dbs=False):
        warnings.warn(DeprecationWarning("redis.flush() has been deprecated, " "use redis.flushdb() or redis.flushall() instead"))
    ...
    def set(self, key, value, expire=None, pexpire=None, only_if_not_exists=False, only_if_exists=False):
        ...
        if only_if_not_exists and only_if_exists:
            raise RedisError("only_if_not_exists and only_if_exists " "cannot be true simultaneously")
    ...
    def pop(self, key, tail=False):
        warnings.warn(DeprecationWarning("redis.pop() has been deprecated, " "user redis.lpop() or redis.rpop() instead"))
    ...
    def zincrby(self, key, increment, member):
        ...
        """
        return self.execute_command("ZINCRBY", key, increment, member).addCallback(self._handle_zincrby)
    ...
    def _zrangebyscore(self, key, min, max, withscores, offset, count, rev):
        ...
        if (offset is None) != (count is None):  # XNOR
            return defer.fail(InvalidData("Invalid count and offset arguments to %s" % cmd))
        if withscores:
    ...
    def zscore(self, key, element):
        ...
        set at key
        """
        return self.execute_command("ZSCORE", key, element).addCallback(self._handle_zscore)
            ...
            if aggregate is min:
                ...
                if isinstance(aggregate, six.string_types):
                    ...
                    if aggregate_u in ("MIN", "MAX", "SUM"):
                        ...
                        err_flag = False
                if err_flag:
                    return defer.fail(InvalidData("Invalid aggregate function: %s" % aggregate))
    ...
    # Sorting
    def sort(self, key, start=None, end=None, by=None, get=None, desc=None, alpha=False, store=None):
    ...
    def execute_pipeline(self):
        ...
        d = defer.DeferredList(deferredList=self.pipelined_replies, fireOnOneErrback=True, consumeErrors=True)
    ...
    def eval(self, script, keys=[], args=[]):
        ...
        if h in self.script_hashes:
            return self.evalsha(h, keys, args).addErrback(self._evalsha_failed, script, h, keys, args)
    ...
    def _evalsha_errback(self, err, script_hash):
        if err.check(ResponseError):
            if err.value.args[0].startswith(u"NOSCRIPT"):
                ...
                raise ScriptDoesNotExist("No script matching hash: %s found" % script_hash)
        return err
    ...
    def script_exists(self, *hashes):
        return self.execute_command("SCRIPT", "EXISTS", post_proc=self._script_exists_success, *hashes)
    ...
    def script_flush(self):
        return self.execute_command("SCRIPT", "FLUSH").addCallback(self._script_flush_success)
    ...
    def _parse_sentinel_state(self, state_array):
        as_dict = dict((self.tryConvertData(key), self.tryConvertData(value)) for key, value in zip(state_array[::2], state_array[1::2]))
    ...
    def sentinel_slaves(self, service_name):
        ...
        return self.execute_command("SENTINEL", "SLAVES", service_name).addCallback(convert)
...
class HiredisProtocol(BaseRedisProtocol):
    def __init__(self, *args, **kwargs):
        ...
        self._reader = hiredis.Reader(protocolError=InvalidData, replyError=ResponseError)
...
class ConnectionHandler(object):
    ...
    def __repr__(self):
        try:
            ...
            return "<Redis Connection: %s:%s - %d connection(s)>" % (cli.host, cli.port, self._factory.size)
...
class UnixConnectionHandler(ConnectionHandler):
    def __repr__(self):
        try:
            ...
            return "<Redis Unix Connection: %s - %d connection(s)>" % (cli.name, self._factory.size)
...
class HashRing(object):
    ...
    def add_node(self, node):
        ...
        for x in range(self.replicas):
            ...
            if isinstance(uuid, six.text_type):
                uuid = uuid.encode()
            crckey = zlib.crc32(six.b(":").join([uuid, str(x).format().encode()]))
    ...
    def remove_node(self, node):
        ...
        for x in range(self.replicas):
            crckey = zlib.crc32(six.b(":").join([node, str(x).format().encode()]))
    ...
    def __repr__(self):
        ...
        for conn in self._ring.nodes:
            try:
                ...
                nodes.append(six.b("%s:%s/%d") % (cli.host, cli.port, conn._factory.size))
    ...
    def __repr__(self):
        ...
        for conn in self._ring.nodes:
            try:
                ...
                nodes.append(six.b("%s/%d") % (cli.name, conn._factory.size))
        return "<Redis Sharded Connection: %s>" % ", ".join(nodes)
...
class RedisFactory(protocol.ReconnectingClientFactory):
    ...
    def __init__(self, uuid, dbid, poolsize, isLazy=False, handler=ConnectionHandler, charset="utf-8", password=None, replyTimeout=None, convertNumbers=True):
    ...
    def buildProtocol(self, addr):
        p = self.protocol(self.charset, replyTimeout=self.replyTimeout, password=self.password, dbid=self.dbid, convertNumbers=self.convertNumbers)
        p.factory = self
...
def makeConnection(host, port, ssl, dbid, poolsize, reconnect, isLazy, charset, password, connectTimeout, replyTimeout, convertNumbers):
    uuid = "%s:%d" % (host, port)
    factory = RedisFactory(uuid, dbid, poolsize, isLazy, ConnectionHandler, charset, password, replyTimeout, convertNumbers)
...
def makeShardedConnection(hosts, ssl, dbid, poolsize, reconnect, isLazy, charset, password, connectTimeout, replyTimeout, convertNumbers):
    ...
    for item in hosts:
        ...
        c = makeConnection(host, port, ssl, dbid, poolsize, reconnect, isLazy, charset, password, connectTimeout, replyTimeout, convertNumbers)
...
def Connection(
    ...
    return makeConnection(host, port, ssl, dbid, 1, reconnect, False, charset, password, connectTimeout, replyTimeout, convertNumbers)
...
def lazyConnection(
    ...
    return makeConnection(host, port, ssl, dbid, 1, reconnect, True, charset, password, connectTimeout, replyTimeout, convertNumbers)
...
def ConnectionPool(
    ...
    return makeConnection(host, port, ssl, dbid, poolsize, reconnect, False, charset, password, connectTimeout, replyTimeout, convertNumbers)
...
def lazyConnectionPool(
    ...
    return makeConnection(host, port, ssl, dbid, poolsize, reconnect, True, charset, password, connectTimeout, replyTimeout, convertNumbers)


def ShardedConnection(hosts, ssl=False, dbid=None, reconnect=True, charset="utf-8", password=None, connectTimeout=None, replyTimeout=None, convertNumbers=True):
    return makeShardedConnection(hosts, ssl, dbid, 1, reconnect, False, charset, password, connectTimeout, replyTimeout, convertNumbers)


def lazyShardedConnection(
    hosts, ssl=False, dbid=None, reconnect=True, charset="utf-8", password=None, connectTimeout=None, replyTimeout=None, convertNumbers=True
):
    return makeShardedConnection(hosts, ssl, dbid, 1, reconnect, True, charset, password, connectTimeout, replyTimeout, convertNumbers)


def ShardedConnectionPool(
    hosts, ssl=False, dbid=None, poolsize=10, reconnect=True, charset="utf-8", password=None, connectTimeout=None, replyTimeout=None, convertNumbers=True
):
    return makeShardedConnection(hosts, ssl, dbid, poolsize, reconnect, False, charset, password, connectTimeout, replyTimeout, convertNumbers)


def lazyShardedConnectionPool(
    hosts, ssl=False, dbid=None, poolsize=10, reconnect=True, charset="utf-8", password=None, connectTimeout=None, replyTimeout=None, convertNumbers=True
):
    return makeShardedConnection(hosts, ssl, dbid, poolsize, reconnect, True, charset, password, connectTimeout, replyTimeout, convertNumbers)


def makeUnixConnection(path, dbid, poolsize, reconnect, isLazy, charset, password, connectTimeout, replyTimeout, convertNumbers):
    factory = RedisFactory(path, dbid, poolsize, isLazy, UnixConnectionHandler, charset, password, replyTimeout, convertNumbers)
    factory.continueTrying = reconnect
    for x in range(poolsize):
...
def makeShardedUnixConnection(paths, dbid, poolsize, reconnect, isLazy, charset, password, connectTimeout, replyTimeout, convertNumbers):
    ...
    connections = []
    for path in paths:
        c = makeUnixConnection(path, dbid, poolsize, reconnect, isLazy, charset, password, connectTimeout, replyTimeout, convertNumbers)
        connections.append(c)
...
def UnixConnection(
    path="/tmp/redis.sock", dbid=None, reconnect=True, charset="utf-8", password=None, connectTimeout=None, replyTimeout=None, convertNumbers=True
):
    return makeUnixConnection(path, dbid, 1, reconnect, False, charset, password, connectTimeout, replyTimeout, convertNumbers)


def lazyUnixConnection(
    path="/tmp/redis.sock", dbid=None, reconnect=True, charset="utf-8", password=None, connectTimeout=None, replyTimeout=None, convertNumbers=True
):
    return makeUnixConnection(path, dbid, 1, reconnect, True, charset, password, connectTimeout, replyTimeout, convertNumbers)


def UnixConnectionPool(
    path="/tmp/redis.sock", dbid=None, poolsize=10, reconnect=True, charset="utf-8", password=None, connectTimeout=None, replyTimeout=None, convertNumbers=True
):
    return makeUnixConnection(path, dbid, poolsize, reconnect, False, charset, password, connectTimeout, replyTimeout, convertNumbers)


def lazyUnixConnectionPool(
    path="/tmp/redis.sock", dbid=None, poolsize=10, reconnect=True, charset="utf-8", password=None, connectTimeout=None, replyTimeout=None, convertNumbers=True
):
    return makeUnixConnection(path, dbid, poolsize, reconnect, True, charset, password, connectTimeout, replyTimeout, convertNumbers)


def ShardedUnixConnection(paths, dbid=None, reconnect=True, charset="utf-8", password=None, connectTimeout=None, replyTimeout=None, convertNumbers=True):
    return makeShardedUnixConnection(paths, dbid, 1, reconnect, False, charset, password, connectTimeout, replyTimeout, convertNumbers)


def lazyShardedUnixConnection(paths, dbid=None, reconnect=True, charset="utf-8", password=None, connectTimeout=None, replyTimeout=None, convertNumbers=True):
    return makeShardedUnixConnection(paths, dbid, 1, reconnect, True, charset, password, connectTimeout, replyTimeout, convertNumbers)


def ShardedUnixConnectionPool(
    paths, dbid=None, poolsize=10, reconnect=True, charset="utf-8", password=None, connectTimeout=None, replyTimeout=None, convertNumbers=True
):
    return makeShardedUnixConnection(paths, dbid, poolsize, reconnect, False, charset, password, connectTimeout, replyTimeout, convertNumbers)


def lazyShardedUnixConnectionPool(
    paths, dbid=None, poolsize=10, reconnect=True, charset="utf-8", password=None, connectTimeout=None, replyTimeout=None, convertNumbers=True
):
    return makeShardedUnixConnection(paths, dbid, poolsize, reconnect, True, charset, password, connectTimeout, replyTimeout, convertNumbers)
...
class SentinelConnectionFactory(RedisFactory):
    ...
    def try_to_connect(self, connector, force_master=False, nodelay=False):
        ...
        def on_master_addr(addr):
            if self._current_master_addr is not None and self._current_master_addr != addr:
                self.resetDelay()
        ...
        if self.is_master or force_master:
            self.sentinel_manager.discover_master(self.service_name).addCallbacks(on_master_addr, on_discovery_err)
        else:
            self.sentinel_manager.discover_slaves(self.service_name).addCallback(on_slave_addrs)
...
class Sentinel(object):
    ...
    def disconnect(self):
        return defer.gatherResults([sentinel.disconnect() for sentinel in self.sentinels], consumeErrors=True)
    ...
    def discover_master(self, service_name):
        ...
        def on_timeout():
            if not result.called:
                result.errback(MasterNotFoundError("No master found for {0}".format(service_name)))
    ...
    def filter_slaves(slaves):
        ...
        return [(slave["ip"], int(slave["port"])) for slave in slaves if not slave["is_odown"] and not slave["is_sdown"]]
    ...
    def master_for(self, service_name, factory_class=SentinelConnectionFactory, dbid=None, poolsize=1, **connection_kwargs):
        factory = factory_class(sentinel_manager=self, service_name=service_name, is_master=True, uuid=None, dbid=dbid, poolsize=poolsize, **connection_kwargs)
        return self._connect_factory_and_return_handler(factory, poolsize)

    def slave_for(self, service_name, factory_class=SentinelConnectionFactory, dbid=None, poolsize=1, **connection_kwargs):
        factory = factory_class(sentinel_manager=self, service_name=service_name, is_master=False, uuid=None, dbid=dbid, poolsize=poolsize, **connection_kwargs)
        return self._connect_factory_and_return_handler(factory, poolsize)
